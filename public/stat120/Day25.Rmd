...
.bqt.font90[Simple linear regression is a powerful tool for understanding the relationship between two quantitative variables]

.bql[
- Mean of $\mathrm{Y}$ is a linear function of $\mathrm{X}$
\begin{align*}
\mu(Y \mid X)=\beta_0+\beta_1 X
\end{align*}
- Model SD is $\boldsymbol{\sigma}$ for all values of $X$.
]
...
.bold.purple-h[How much of an effect?]
...
- linearity, normality, constant variance, independent observation
