---
title: "Binomial Logistic regression: deviance"
...
.blockquote[
.bold[Probability:]
- This assumption implies that the probability $\pi\left(\right.$area$\left._{i}\right)$ of extinction on island $i$ is the same for all at risk species on island $i$.
...
.code70[
```{r, fig.width=3, fig.height=2.5, out.width="30%", fig.align='center', collapse=TRUE}
...
.pull-right-60[
```{r, collapse=TRUE, fig.width=5, fig.height=4}
...
.code70[
```{r,fig.width=5, fig.height=3.5, out.width="60%", fig.align='center', collapse=TRUE}
library(GGally)
...
```{r, echo=FALSE}
library(ggrepel)
p1 <- ggplot(island_aug, aes(x = Area, y = AtRisk)) +
  scale_x_log10() +
  geom_point(aes(color = .hat, size = .hat)) +  # size of the points
  scale_color_continuous(low = "green", high = "red") +   
  guides( size = FALSE)
  
p1 +  geom_label_repel(aes(label = ID),
                  box.padding   = 0.35, 
                  point.padding = 0.5,
...
```{r, echo=FALSE}
p2 <- ggplot(island_aug, aes(x = Area, y = AtRisk)) +
  scale_x_log10() +
  geom_point(aes(color = .cooksd, size = .cooksd)) +  # size of the points
  scale_color_continuous(low = "green", high = "red") +   
  guides( size = FALSE)
  
p2 +  geom_label_repel(aes(label = ID),
                  box.padding   = 0.35, 
                  point.padding = 0.5,
                  segment.color = 'grey50')
...
.pull-right[
```{r data-viz, echo=FALSE, fig.align='center'}
island <- island %>% mutate(ID = row_number())
p3 <- ggplot(island, aes(x=log(Area), 
                         y = Extinct/AtRisk,
                         weight = AtRisk)) +
 geom_point() +
 geom_smooth(method="glm", se=FALSE,
...
p3 +  geom_label_repel(aes(label = ID),
                  box.padding   = 0.15, 
                  point.padding = 0.3,
                  segment.color = 'grey50')
```
