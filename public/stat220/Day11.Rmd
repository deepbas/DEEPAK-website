...
kanye <- read_csv("/home/deepak/Desktop/scratch/tweets/Kanye_cleaned.csv")
...
.large.out-t[pulling apart some text or `string` to do something with it]
...
- .Large[The most common tasks in string processing include:]
  ...
  + .yellow-h[extracting] specific parts of strings, e.g. .yellow-h["Temp 40 F$"]
  + .yellow-h[splitting] strings into multiple values, e.g. .yellow-h["Boston, MA"]
]
...
.large.out-t[Regular expressions are a language for expressing patterns in strings]
...
.bq[
.large[We can manage the lengths of strings using the following set of functions:]
...
> .large[allows one to interpolate strings and values that have been assigned to names in R]
