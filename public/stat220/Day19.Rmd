...
.center.font200["🖱️ A button that lets users decide when to make things happen."]
...
```r
ui <- fluidPage(
  numericInput("numInput", "Enter a number:", 1),
...
server <- function(input, output) {
  ...
  output$incrementedValue <- renderText({
    paste("Incremented value:", values$num)
  })
}
shinyApp(ui, server)
```
