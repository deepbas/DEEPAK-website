...
`r chunk_reveal("barplot", font_size_code="70%", width= c(0.5, 0.5), title = "## Bar Plot")`
...
ggplot(mtcars, aes(x = factor(cyl))) +
  ...
  theme_minimal() +
  labs(title = "Count of Cars by Cylinder Count", x = "Number of Cylinders", y = "Count")
```
...
ggplot(mtcars, aes(x = factor(cyl), y = mpg)) +
  ...
  theme_minimal() +
  labs(title = "Miles per Gallon (mpg) by Cylinder Count", x = "Number of Cylinders", y = "mpg")
```
...
`r chunk_reveal("QQ-plot", font_size_code="70%", width= c(0.5, 0.5), title = "## QQ plot")`

```{r QQ-plot, fig.align='center', echo=FALSE, out.width="100%", eval=FALSE}
...
ggplot(mtcars, aes(x = factor(cyl), y = mpg)) +
  ...
  theme_minimal() +
  labs(title = "Mean mpg by Number of Cylinders", x = "Number of Cylinders", y = "Mean mpg")
```


---

`r chunk_reveal("ebars-plot", font_size_code="70%", width= c(0.5, 0.5), title = "## Error Bars Line plot")`

```{r ebars-plot, fig.align='center', echo=FALSE, out.width="100%", eval=FALSE}
...
`r chunk_reveal("layering-geoms", font_size_code="70%", width= c(0.5, 0.5), title = "## Layering geoms with patchwork")`
...
# Layering geoms
p1 <- ggplot(hate_crimes, aes(x = median_household_income,
                              y = hate_crimes_per_100k_splc)) +
  geom_point() +
  labs(y = "hate crimes", x = "household income")
...
`r chunk_reveal("ggradar", font_size_code="70%", width= c(0.5, 0.5), title = "## Radar Chart with ggradar")`

```{r ggradar, fig.align='center', echo=FALSE, out.width="100%", eval=FALSE}
library(ggradar)
library(scales) # to automatically match the scales
...
# Radar chart
ggradar(radar_data, grid.min = 0, 
        grid.mid = 0.5, 
        grid.max = 1,
        axis.label.size = 3,
        grid.label.size = 3) +
  theme_minimal() +
  labs(title = "Radar Chart with ggradar")
```
...
## Multi-panel plots in a grid using `gridExtra`
...
`r countdown(minutes = 20, seconds = 00, top = 0 , color_background = "inherit", padding = "3px 4px", font_linewidth = "2em")`
