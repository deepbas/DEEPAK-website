...
```{r}
set.seed(1234)
res_kmeans <- kmeans(my_df, centers = 2, nstart = 25)
```
...
```{r}
multi_kmeans <- tibble(k = seq(1,15) ) %>%
  mutate(
    model = purrr::map(k, ~ kmeans(my_df, .x, nstart = 25)),
