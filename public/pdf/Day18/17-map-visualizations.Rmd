...
Interactive App using `ggiraph` to Shiny
...
server <- function(input, output) {
  ...
  geom_polygon_interactive(aes(fill = covid_data[[input$variable]], group = group,
                               tooltip = tooltip)) +
  guides(fill=guide_legend(title="Cases")) + scale_fill_fermenter(type = "div", palette = "Spectral")
...
library(htmlwidgets)
saveWidget(n, file="Minnesota.html")
