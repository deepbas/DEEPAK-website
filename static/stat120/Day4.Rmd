...
.bq.font90[A difference in proportions is a difference in proportions for one categorical variable calculated for different levels of the other categorical variable]

 - Example: Difference in proportions of male and female that are in a relationship:
 
\begin{align}
  \text{proportion}_{\text{Females}} - \text{proportion}_{\text{Males}} = 
...
round(prop.diff,2) # round the final answer to 2 significant digits
```
...
- Let's go over a tutorial on data visualization in R using `ggplot2` on [course helper page](https://stat120-fall23.netlify.app/tutorial-2-dive-deeper-into-data-visualization-and-statistics-in-r)
]
]
...
`r chunk_reveal("bar-plot", font_size_code="50%",widths = c(0.6, 0.4), title = "## Graphical summary: bar plot using ggplot2")`
