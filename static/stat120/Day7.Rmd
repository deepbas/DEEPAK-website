...
`r chunk_reveal("bootstrap", widths = c(0.6, 0.4), title = "## Bootstrap Distrubution")`



.font70[
```{r bootstrap, fig.width = 4, fig.height = 3.5, out.width = "100%", include=FALSE}
set.seed(143); (X <- c(20,24,19,23,22,16)) # n = 6
(X1_star <- sample(X, replace = TRUE))
(X2_star <- sample(X, replace = TRUE))
(X3_star <- sample(X, replace = TRUE))
(X4_star <- sample(X, replace = TRUE))
(X5_star <- sample(X, replace = TRUE))
(XN_star <- sample(X, replace = TRUE))
(mean(X))
(mean(X1_star))
(mean(X2_star))
(mean(X3_star))
(mean(X4_star))
(mean(X5_star))
(mean(XN_star))  # N = simulation size
```
]

---

`r chunk_reveal("bootstrap-demo", widths = c(0.52, 0.48), title = "## Bootstrap Distrubution")`

.font70[
```{r bootstrap-demo, fig.width = 5, fig.height = 5, out.width = "100%", include=FALSE}
library(purrr)
set.seed(143) 
X <- c(20,24,19,23,22,16)

# Prototype simulation with N = 500
