...
- For both types of test, large chi-square test stat values support the .bold[alternative] hypothesis so: $p-value = P(\chi^2 \geq \text{observed } \chi^2)$
...
```{r, collapse=TRUE}
table(survey$Question.8, survey$Question.9)
```
...
.out-t[ Observed distribution of comfort level given religiousness]

```{r, collapse=TRUE}
table(survey$religiousness, survey$comfortness)
```
...
.bql[
- Skim through Example 1 in today's class activity
]
]
